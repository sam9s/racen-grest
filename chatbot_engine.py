...
from safety_guardrails import (
    apply_safety_filters, get_system_prompt, filter_response_for_safety, 
    inject_product_links, append_contextual_links, inject_specific_product_links,
    add_navigation_prompt, detect_navigation_confirmation, detect_language
)
...
# Feature flag to disable auto-navigation feature (can be re-enabled later)
FEATURE_NAVIGATION_ENABLED = False
...
# Session-level pending navigation URL tracking
# Key: session_id, Value: URL to navigate to when user confirms
_session_pending_navigation = {}
...
def set_pending_navigation(session_id: str, url: str):
    """Set a pending navigation URL for the session."""
    if session_id and url:
        _session_pending_navigation[session_id] = url


def get_pending_navigation(session_id: str) -> str:
    """Get the pending navigation URL for the session."""
    return _session_pending_navigation.get(session_id)


def clear_pending_navigation(session_id: str):
    """Clear the pending navigation URL for the session."""
    if session_id in _session_pending_navigation:
        del _session_pending_navigation[session_id]


def extract_first_product_url(response: str) -> str:
    """Extract the first product URL from a response containing markdown links."""
    import re
    match = re.search(r'\[[^\]]+\]\((https://grest\.in/products/[^)]+)\)', response)
    if match:
...
IMPORTANT: Only use information from the context above. If the answer is not in the context, politely say you don't have that specific information and offer to help them contact us at https://grest.in/pages/contact-us"""
    ...
    try:
        ...
        response_with_specific_links = inject_specific_product_links(response_with_product_links)
        
        # Navigation prompt (disabled via feature flag)
        if FEATURE_NAVIGATION_ENABLED:
            user_language = detect_language(user_message)
            response_with_nav_prompt = add_navigation_prompt(response_with_specific_links, user_language)
        else:
            response_with_nav_prompt = response_with_specific_links
        
        final_response = append_contextual_links(user_message, response_with_nav_prompt)
...
def generate_response_stream(
    ...
    """
    # Navigation confirmation handling (disabled via feature flag)
    if FEATURE_NAVIGATION_ENABLED:
        pending_url = get_pending_navigation(session_id) if session_id else None
        if pending_url and detect_navigation_confirmation(user_message):
            clear_pending_navigation(session_id)
            user_lang = detect_language(user_message)
            nav_response = "Taking you to that page now!" if user_lang == "english" else "Aapko us page par le ja raha hoon!"
            yield {"type": "content", "content": nav_response}
            yield {
                "type": "done",
                "full_response": nav_response,
                "sources": [],
                "safety_triggered": False,
                "navigate_to": pending_url
            }
            return
...
IMPORTANT: Only use information from the context above. If the answer is not in the context, politely say you don't have that specific information and offer to help them contact us at https://grest.in/pages/contact-us"""
    ...
    try:
        ...
        response_with_specific_links = inject_specific_product_links(response_with_links)
        ...
        # Pending navigation tracking (disabled via feature flag)
        if FEATURE_NAVIGATION_ENABLED:
            product_url = extract_first_product_url(final_response)
            if session_id and product_url:
                set_pending_navigation(session_id, product_url)
